...
def _mk_model() -> GraphModel:
    m = GraphModel()
    m.add_node(Node(
        id="mic", kind=NodeKind.HW_SOURCE, label="Mic",
        ports=[Port("out", PortKind.SOURCE_OUT, "out")],
    ))
    m.add_node(Node(
        id="vmic", kind=NodeKind.NULL_SINK, label="VMic",
        ports=[
    ...
    m.add_node(Node(
        id="spk", kind=NodeKind.HW_SINK, label="Speakers",
...
class TestCanConnect:
    def test_source_out_to_sink_in_ok(self):
        assert can_connect(_mk_model(), PortHit("mic", "out"), PortHit("vmic", "in"))

    def test_monitor_out_to_sink_in_ok(self):
    ...
    def test_sink_in_to_source_out_accepted(self):
    ...
    def test_two_producers_rejected(self):
        ...
        assert not can_connect(
            _mk_model(), PortHit("mic", "out"), PortHit("vmic", "monitor"),
        )
...
class TestDragMachine:
    def test_begin_on_port_starts_edge_drag(self):
    ...
    def test_begin_on_node_starts_node_drag(self):
        dm = DragMachine(_mk_model())
        kind = dm.begin(
            NodeHit("mic"), pointer_x=120.0, pointer_y=60.0,
    ...
    def test_begin_on_edge_or_blank_resets(self):
        dm = DragMachine(_mk_model())
    ...
    def test_node_drag_update_tracks_grab_offset(self):
        dm = DragMachine(_mk_model())
    ...
    def test_node_drag_end_emits_move_intent(self):
        dm = DragMachine(_mk_model())
        dm.begin(NodeHit("mic"), 120.0, 60.0, node_origin=(100.0, 50.0))
    ...
    def test_edge_drag_drop_on_compatible_port_emits_connect(self):
        dm = DragMachine(_mk_model())
        dm.begin(PortHit("mic", "out"), 0, 0)
        intent = dm.end(drop_hit=PortHit("vmic", "in"))
        assert intent == ConnectIntent("mic", "out", "vmic", "in")

    def test_edge_drag_from_sink_to_producer_normalizes_direction(self):
    ...
    def test_edge_drag_drop_on_incompatible_returns_none(self):
        dm = DragMachine(_mk_model())
    ...
    def test_edge_drag_drop_on_blank_returns_none(self):
        dm = DragMachine(_mk_model())
        dm.begin(PortHit("mic", "out"), 0, 0)
