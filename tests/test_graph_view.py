...
def _populated_model() -> GraphModel:
...
def test_signals_mirror_model_changes() -> None:
    from audio_spider.graph_model import (
        Edge,
        Node,
        NodeKind,
...
def test_node_item_port_position_is_absolute() -> None:
    from audio_spider.graph_view import GraphView
    ...
    assert sink_item is not None
    sx, _sy = src_item.port_position("out")
    dx, _dy = sink_item.port_position("in")
...
def test_bezier_between_starts_and_ends_at_endpoints() -> None:
...
def _menu_item_labels(menu: Any) -> list[str]:
...
def _populated_view_with_controller(
    tmp_path: Path,
) -> tuple[GraphView, GraphModel, Controller]:
    ...
    ctrl = Controller(
        ...
        cfg,
        model,
        config_path=tmp_path / "port_menu.json",
    )
    ctrl.initial_sync()
...
def test_port_menu_on_source_out_offers_connect_targets(tmp_path: Path) -> None:
    from gi.repository import Gtk

    from audio_spider.drag_state import PortHit

    view, _model, _ctrl = _populated_view_with_controller(tmp_path)
    menu = Gtk.Menu()
    ...
    assert any("Virtual Mic 1" in lbl and "in" in lbl for lbl in labels), labels


def test_port_menu_on_source_out_shows_disconnect_for_existing(tmp_path: Path) -> None:
    from gi.repository import Gtk

    from audio_spider.drag_state import PortHit

    view, _model, ctrl = _populated_view_with_controller(tmp_path)
    ctrl.request_connect("mic_a", "out", "vmic1", "in")
    menu = Gtk.Menu()
    view._build_port_menu(menu, PortHit("mic_a", "out"))
    labels = _menu_item_labels(menu)
    assert any(lbl.startswith("Disconnect from") for lbl in labels), labels
    # connect submenu still present but does NOT list vmic1.in anymore
    sub_labels = [lbl for lbl in labels if lbl.startswith("  > ")]
    assert not any("Virtual Mic 1" in lbl for lbl in sub_labels), sub_labels


def test_port_menu_on_sink_in_offers_disconnect_only(tmp_path: Path) -> None:
    from gi.repository import Gtk
    ...
    labels = _menu_item_labels(menu)
    disconnects = [lbl for lbl in labels if lbl.startswith("Disconnect from")]
    assert len(disconnects) == 2, labels
    # no Connect submenu on a sink-in port (the natural flow is drag-from-output)
    assert not any(lbl == "Connect to" for lbl in labels), labels


def test_port_menu_on_empty_combine_members_offers_add(tmp_path: Path) -> None:
    ...
    labels = _menu_item_labels(menu)
    assert any(lbl == "Add member" for lbl in labels), labels
    sub_labels = [lbl for lbl in labels if lbl.startswith("  > ")]
    # the hw sink in the fixture is "Speakers"
    assert any("Speakers" in lbl for lbl in sub_labels), sub_labels


def test_port_menu_on_combine_members_shows_remove_for_existing(tmp_path: Path) -> None:
    from gi.repository import Gtk
    ...
    view._build_port_menu(menu, PortHit("vout", "members"))
    labels = _menu_item_labels(menu)
    assert any(lbl.startswith("Remove member: ") for lbl in labels), labels


def test_port_menu_add_member_invokes_controller(tmp_path: Path) -> None:
    from gi.repository import Gtk

    from audio_spider.drag_state import PortHit

    view, model, ctrl = _populated_view_with_controller(tmp_path)
    ...
    target = next(c for c in submenu.get_children() if c.get_label() and "Speakers" in c.get_label())
    target.emit("activate")
    members = {e.dst_node for e in model.edges() if e.kind == "combine-member"}
    assert "speakers" in members


def test_port_menu_remove_member_invokes_controller(tmp_path: Path) -> None:
    from gi.repository import Gtk
    ...
    menu = Gtk.Menu()
    view._build_port_menu(menu, PortHit("vout", "members"))
    remove_item = next(c for c in menu.get_children() if c.get_label() and c.get_label().startswith("Remove member: "))
    remove_item.emit("activate")
    assert not any(e.kind == "combine-member" and e.dst_node == "speakers" for e in model.edges())


def test_port_menu_disconnect_invokes_controller(tmp_path: Path) -> None:
    from gi.repository import Gtk
    ...
    view, model, ctrl = _populated_view_with_controller(tmp_path)
    ctrl.request_connect("mic_a", "out", "vmic1", "in")
    edge_before = next(iter(model.edges()))

    menu = Gtk.Menu()
    view._build_port_menu(menu, PortHit("mic_a", "out"))
    disconnect_item = next(
        c for c in menu.get_children() if c.get_label() and c.get_label().startswith("Disconnect from")
    )
...
def test_port_menu_connect_invokes_controller(tmp_path: Path) -> None:
    from gi.repository import Gtk

    from audio_spider.drag_state import PortHit

    view, model, _ctrl = _populated_view_with_controller(tmp_path)
    menu = Gtk.Menu()
    view._build_port_menu(menu, PortHit("mic_a", "out"))
    connect_root = next(c for c in menu.get_children() if c.get_label() == "Connect to")
    submenu = connect_root.get_submenu()
    target_item = next(c for c in submenu.get_children() if c.get_label() and "Virtual Mic 1" in c.get_label())
    target_item.emit("activate")
    edges = [e for e in model.edges() if e.kind == "loopback"]
    assert any(e.src_node == "mic_a" and e.dst_node == "vmic1" for e in edges), edges
