...
from audio_spider.pa_backend import PABackend, PAModule
...
class FakePA:
    ...
    def load_null_sink(self, name: str, description: str | None = None) -> int:
        ...
        if name in self.fail_on:
            msg = f"simulated failure: {name}"
            raise PABackendError(msg)
        return self._record(
            "module-null-sink",
            f"sink_name={name}" + (f" sink_properties=device.description={description}" if description else ""),
        )
    ...
    def load_combine_sink(
        ...
        self.calls.append(("combine-sink", (name, tuple(members)), {"description": description}))
        if name in self.fail_on:
            msg = f"simulated failure: {name}"
            raise PABackendError(msg)
        return self._record(
    ...
    def load_loopback(self, source: str, sink: str, latency_msec: int = 1) -> int:
        self.calls.append(("loopback", (source, sink), {"latency_msec": latency_msec}))
...
class TestReconcile:
    def test_loads_missing_null_sink(self) -> None:
        pa = FakePA()
        cfg = Config(
            modules=[
                ConfigModule(id="vmic1", kind="null-sink", params={"name": "vmic1"}),
            ]
        )
        report = reconcile(cfg, cast("PABackend", pa))
        assert report.created == {"vmic1": 0}
        assert report.skipped == {}
        assert report.errors == {}
        assert report.ok

    def test_skips_existing_null_sink(self) -> None:
        pa = FakePA(
            modules=[
    ...
    def test_skips_existing_loopback(self) -> None:
        pa = FakePA(
            modules=[
                PAModule(index=7, name="module-loopback", argument="source=mic_a sink=vmic1 latency_msec=5"),
            ]
        )
        cfg = Config(
            modules=[
                ConfigModule(id="lb1", kind="loopback", params={"source": "mic_a", "sink": "vmic1"}),
            ]
        )
        report = reconcile(cfg, cast("PABackend", pa))
        assert report.skipped == {"lb1": 7}

    def test_signature_match_for_combine_sink_order_insensitive(self) -> None:
        pa = FakePA(
            modules=[
                PAModule(index=3, name="module-combine-sink", argument="sink_name=split slaves=b,a"),
            ]
        )
        cfg = Config(
            modules=[
                ConfigModule(id="split", kind="combine-sink", params={"name": "split", "members": ["a", "b"]}),
            ]
        )
        report = reconcile(cfg, cast("PABackend", pa))
        assert report.skipped == {"split": 3}

    def test_leaves_extras_alone(self) -> None:
        # PA has an unrelated module that's not in cfg → reconcile must NOT touch it
        pa = FakePA(
            modules=[
                PAModule(index=1, name="module-null-sink", argument="sink_name=someone_elses_sink"),
            ]
        )
        cfg = Config(
            modules=[
                ConfigModule(id="mine", kind="null-sink", params={"name": "my_sink"}),
            ]
        )
        report = reconcile(cfg, cast("PABackend", pa))
    ...
    def test_records_error_on_load_failure(self) -> None:
        pa = FakePA()
        pa.fail_on = {"broken"}
        cfg = Config(
            modules=[
                ConfigModule(id="vmic1", kind="null-sink", params={"name": "broken"}),
                ConfigModule(id="vmic2", kind="null-sink", params={"name": "ok"}),
            ]
        )
        report = reconcile(cfg, cast("PABackend", pa))
    ...
    def test_invalid_config_module_recorded_as_error(self) -> None:
        pa = FakePA()
        cfg = Config(
            modules=[
    ...
    def test_unknown_kind_is_error_via_signature(self) -> None:
        pa = FakePA()
        cfg = Config(
            modules=[
    ...
    def test_id_to_module_index_merges_created_and_skipped(self) -> None:
        pa = FakePA(
            modules=[
    ...
    def test_subsequent_entries_see_just_loaded_modules(self) -> None:
